...
def loadPage(url):
    ...
    try:
        ...
        if check(driver,XPATHS.BUTTONS.OK):
            ok_button=driver.find_element_by_xpath(XPATHS.BUTTONS.OK)
            clickLink(ok_button)
            waitSomeTime(mult=2)
        # chat button
        if check(driver,XPATHS.BUTTONS.CHAT):
            chat_close=driver.find_element_by_xpath(XPATHS.BUTTONS.CHAT)
            clickLink(chat_close)
            waitSomeTime(mult=2)
    except  Exception as e:
        print(e)